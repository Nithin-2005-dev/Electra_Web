...
export async function POST(req) {
  try {
    ...
    for (const docSnap of snap.docs) {
      ...
      if (!deliverableItems.length) continue;

      /* ───────── UPDATE ITEMS (NO TIMESTAMPS IN ARRAY) ───────── */
      ...
      /* ───────── BUILD EMAIL ITEMS ───────── */
      const emailItems = deliverableItems.map((i) => ({
      ...
      await resend.emails.send({
        ...
        to: [user.email],
        subject: "Your Electra order item has been delivered",
        html: deliveredEmailHtml(order, emailItems, user),
      });
    }
