...
const AboutUs = () => {
  return (
    <section id='about'>
      <motion.h2 className='text-5xl font-bold m-2 border-b-4 inline-block border-yellow-400 mx-4 text-justify' initial={{y:-10,opacity:0}} whileInView={{y:0,opacity:1,transition:{duration:2}}}>About Us</motion.h2>
      <div className='p-4 font-serif font-medium text-blue-100 text-justify'>{
        ...
        PillarsData.map((pillar,i)=>{
          ...
          <CardImg src={pillar.image} alt={pillar.name} width={200} style={{borderRadius:'10%'}} className='aspect-square'/>
            ...
            <div className='font-bold text-2xl text-fuchsia-200'>{pillar.name}</div>
                ...
                <div className={` ${styles['edu-au-vic-wa-nt-guide ']} text-blue-100 font-semibold text-xs lg:text-lg text-justify`}>{pillar.Info}</div>
            </div>
            {
