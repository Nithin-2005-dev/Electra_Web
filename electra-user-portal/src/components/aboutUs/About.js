...
const AboutUs = () => {
  return (
    <div >
      <motion.div className='text-5xl font-bold m-2 border-b-4 inline-block border-yellow-400 ' initial={{y:-10,opacity:0}} whileInView={{y:0,opacity:1,transition:{duration:2}}}>About Us</motion.div>
      <div className='p-4 font-mono font-medium text-blue-50'>
      ...
      <div className='border-2 m-5 rounded-3xl border-red-300'>
      {
        PillarsData.map((pillar)=>{
            return <><div key={pillar.name} className='flex m-4 flex-col items-center p-5 gap-5 text-center'>
            <div>
            ...
            <div className='font-bold text-2xl text-fuchsia-200'>{pillar.name}</div>
                ...
                <div className={`text-left ${styles['quicksand']} text-blue-100 font-semibold`}>{pillar.Info}</div>
            </div>
