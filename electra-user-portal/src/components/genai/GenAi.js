...
/>
      ...
      <input type="text" placeholder="Type your doubt Here" className='w-[55vw] lg:w-[45vw] rounded-xl p-2 ml-2 mr-1 text-black' ref={queryRef}/>
         ...
         <div className='flex m-2 my-3 justify-between'>
            <button className={`${styles['hvr-ripple-in']} bg-red-500 p-1 px-3 font-bold rounded-xl`}
            onClick={()=>{
                setResponse('')
            }}
            >clear<MdDeleteForever className='inline-block m-1'/></button>
            <button className={`${styles['hvr-ripple-in']} bg-green-500 p-1 px-3 font-bold rounded-xl`} onClick={handleQuery}>Ask<FaRegCircleQuestion className='inline-block m-1'/></button>
            <button className={`${styles['hvr-ripple-in']} bg-blue-500 p-1 px-3 font-bold rounded-xl`} onClick={()=>{
